...
(defun getLeftRightSubtrees (binary_tree no_of_nodes no_of_edges col)
	...
	(cond
		((equal (- no_of_nodes 1) no_of_edges) (cons col (list binary_tree)))
		((null binary_tree) (append (list col) (list binary_tree)))
		((getLeftRightSubtrees (cddr binary_tree) (+ 1 no_of_nodes) (+ (cadr binary_tree) no_of_edges) (append col (cons (car binary_tree) (list (cadr binary_tree))))))
	)
)

(defun getSubtrees (binary_tree)
	(getLeftRightSubtrees (cddr binary_tree) 0 0 nil) ; <=> (getLeftRightSubtrees (cdr (cdr binary_tree)) 0 0 'nil)
)
...
(defun convertBinaryTree (binary_tree)
	(cond
		((endp binary_tree) '())
		((not (cddr binary_tree)) (list (car binary_tree)))
		('t (cons (car binary_tree) (cons (convertBinaryTree (getLeftSubtree binary_tree)) (list (convertBinaryTree (getRightSubtree binary_tree))))))
	)
)